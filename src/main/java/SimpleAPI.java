...
/**
 * A simplified Java API consisting of a single {@link #runJob()}
 * method that accepts a script name and a list of options and returns
 * a {@link Job}. It is used to build a simple Java CLI (see the
 * {@link #main()} method). The simplified API also makes it easier to
 * bridge with other programming languages using JNI.
 */
@Component(
	name = "SimpleAPI",
	immediate = true
...
public class SimpleAPI {
	...
	private void runJob(String scriptName, Map<String,String> options) throws InterruptedException, IOException {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		for (CreateOnStart o : ServiceLoader.load(CreateOnStart.class))
			if (api == null && o instanceof SimpleAPI)
				api = (SimpleAPI)o;
		if (api == null)
			throw new IllegalStateException();
		api.runJob(script, options);
		System.exit(0);
	}
}
